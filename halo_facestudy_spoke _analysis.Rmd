...
boundaryplots$pc1_2<- ggplot(df, aes(PC1, PC2, fill = expression)) +
...
boundaryplots$pc1_3<-ggplot(df, aes(PC1, PC3, fill = expression)) +
    # geom_raster(alpha = 0.5, interpolate=FALSE) +
  # geom_density_2d_filled(show.legend = FALSE,alpha = .1) 
  geom_tile(alpha = 0.01, size = 0) +
  # stat_contour(data = df,aes(x =PC1, y =PC2, z = expression))
...
getleg <-cowplot::get_legend(boundaryplots$pc1_3) 
...
evalm(r.forest_mts$)
res <- evalm(list(fit1,fit2,fit3),gnames=c('1','2','3'))

...
spoken_rf <-randomForest::randomForest(as.factor(expression) ~ PC1 + PC2 + PC3,
  ...
  #              proximity = TRUE)
    ...
    spoken_rf <- train(expression ~ comp1_maxnorm + comp2_maxnorm + 
                         comp3_maxnorm, 
...
x_max_sp <- max(spoken_mts_no0Sd_no0Sd_dcast_2$comp1_maxnorm + 0.5)
...
df_sp$expression <- predict(spoken_rf, df_sp)
...
spoken_mts_no0Sd_no0Sd_dcast_2agg<-spoken_mts_no0Sd_no0Sd_dcast_2%>%
  group_by(expression)%>%
  summarise_if(is.numeric,mean, na.rm=T)

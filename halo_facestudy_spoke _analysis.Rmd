...
as.data.frame(confusionMatrix(test_set$prediction , as.factor(test_set$expression), mode = "everything", positive="1")[["table"]])%>%
  mutate(prop = Freq/15)%>%
      ggplot(aes(Prediction, Reference, fill = prop))+
    geom_tile()+
       # scale_fill_viridis_c(option = "magma")
    scale_fill_viridis_c(option = "magma", breaks=c(0,.5,1),
                       limits =c(0,1))+
...
boundaryplots$pc1_2 <- 
ggplot(df, aes(PC1, PC2, fill = expression)) +
    # geom_raster(alpha = 0.5, interpolate=FALSE) +
  # geom_density_2d_filled(show.legend = FALSE,alpha = .1) 
  geom_tile(alpha = 0.01, size = 0) +
...
#   geom_point(data = bind_train_test, shape = 21, size = 3) +
# 
#   geom_xsidedensity(data = bind_train_test,aes(x =PC1, fill = expression), alpha = 0.5)+
#   geom_ysideboxplot(data = bind_train_test,aes(x =PC2, fill = expression), alpha = 0.5, orientation = "x")
#   
...
# as.factor(bind_train_test$expression)
# 
# boundaryplots$pc1_3 
# 
...
boundaryplots$pc1_3_box <- ggMarginal(boundaryplots$pc1_3,groupFill = T,type = c("boxplot"),
                                       margins = 'both',
  size = 5)
 
boundaryplots$pc1_3_den <- ggMarginal(boundaryplots$pc1_3,groupFill = T, type = "density")
...
# others
roc.sad <- roc(ifelse(predictions$observed=="sad", "sad", "rest"), as.numeric(predictions$sad))
...
roc.sad$sensitivities
roc.happy$sensitivities
roc.angry$sensitivities
...
# mutate(test= contains(,row_name)) %>%
  ...
  group_by(key)%>%
  summarise_if(is.numeric, mean, na.rm = T) %>%
  ggplot(aes(key, (value+.01), color = key)) +
   geom_bar(stat="identity", width = .11)+
  geom_point(aes(x = key, y = (acc_test+.01)))+
  geom_errorbar( aes(x=key, ymin=(perc_5+.01), ymax=(perc_95+.01)), width=0.4, colour="orange", alpha=0.9, size=1.3)
  # coord_cartesian(ylim=c(.7,1))
  # ylim(.30,1)
  facet_grid(~key) +
    geom_vline(aes(xintercept = (perc_5+.01))) +
   geom_vline(aes(xintercept =(perc_95+.1))) +
    geom_vline(aes(xintercept = (acc_test+.1)), linetype ="dashed", color = "red")

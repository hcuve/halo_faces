...
unique(test_ts_dcast$filename)
unique(mts_maxnorm_dcast$filename)
...
# install.packages("caret")
con_mat_posed<- caret::confusionMatrix(test_set$prediction , as.factor(test_set$expression), mode = "everything", positive="1")


 caret::confusionMatrix(as.factor(test_set$expression), as.factor(test_set$prediction), mode = "everything", positive="1")
...
con_mat_posed$overall
con_mat_posed



result.roc <- pROC::roc(as.numeric(as.character(test_set$expression)), as.numeric(test_set$prediction)) # Draw ROC curve.
...
permutation_test_accuracy <- function(predictions, actual_labels, num_permutations = 1000) {
  observed_accuracy <- sum(predictions == actual_labels) / length(actual_labels)
  
  permuted_accuracies <- numeric(num_permutations)
  for (i in 1:num_permutations) {
    permuted_labels <- sample(actual_labels)
    permuted_accuracy <- sum(predictions == permuted_labels) / length(permuted_labels)
    permuted_accuracies[i] <- permuted_accuracy
  }
  
  p_value <- sum(permuted_accuracies >= observed_accuracy) / num_permutations
  return(p_value)
}
...
con_mat_posed
permutation_test_accuracy(test_set$prediction, test_set$expression)
print(p_value)

```

# compute chi sqyare tests for pairwise compartisosn, cpompute wethe the clases are sufficiantly different and 2 which confusions are signficiant
...
# Create the confusion matrix in one line
test_cm <- matrix(c(7, 7, 0, 5, 9, 0, 0, 0, 14), 
                           nrow = 3, 
                           byrow = TRUE,
                           dimnames = list("Reference" = c("angry", "happy", "sad"),
                                           "Prediction" = c("angry", "happy", "sad")))
...
test_csq_pw<-rcompanion::pairwiseNominalIndependence(con_mat_posed$table,
                                        compare = "row",
                                        fisher = TRUE)


test_csq_pw$Comparison
test_csq_pw
test_csq_pw$p.adj.Fisher


```

testn<-RVAideMemoire::chisq.multcomp(as.numeric(con_mat_posed$table+1))
testn$data.name

testn$p.value
test_ts_dcast_no0Sdpairwise_chisq(con_mat_posed$table+1)
...
which(testn$p.value < 0.05, arr.ind = TRUE)

con_mat_posed$table+1
...
"angry : happy" "angry : sad"   "happy : sad"  
[1] 0.0000000493 0.0000105000 0.0000000493
...
as.numeric(con_mat_posed$table+1)

?RVAideMemoire::chisq.multcomp
RVAideMemoire::chisq.multcomp(as.numeric(con_mat_posed$table+1))


counts <- as.numeric(con_mat_posed$table+1)
# Perform pairwise comparisons using chisq.multcomp
pairwise_results <- chisq.multcomp(counts, p.method = "fdr")

# Perform pairwise comparisons using chisq.multcomp
pairwise_results <- chisq.multcomp(counts, p.method = "fdr")

# Print the results with interpretation
pairwise_results$p.value <- matrix(pairwise_results$p.value, nrow = length(counts), dimnames = list(as.character(counts), as.character(counts)))
...
# Format the p-values into a matrix
num_categories <- length(con_mat_posed$table)

p_values_matrix <- matrix(pairwise_results$p.value, nrow = num_categories, byrow = TRUE,
                          dimnames = list(paste("Row", 1:num_categories), paste("Column", 1:num_categories)))
...
# Define the confusion matrix and add 1 to each element to avoid zero counts
con_mat_posed <- matrix(c(12, 0, 0,
                          0, 15, 1,
                          3, 0, 14) + 1, 
                        nrow = 3, 
                        byrow = TRUE,
                        dimnames = list("Prediction" = c("angry", "happy", "sad"),
                                        "Reference" = c("angry", "happy", "sad")))

# Function to perform pairwise chi-squared tests
pairwise_chisq <- function(matrix) {
  categories <- colnames(matrix)
  results <- data.frame(Comparison = character(), p.value = numeric(), stringsAsFactors = FALSE)
  
  for (i in 1:nrow(matrix)) {
    for (j in 1:ncol(matrix)) {
      if (i != j) {
        # Create a 2x2 table for the chi-squared test
        observed <- matrix[c(i, j), c(i, j)]
        test_result <- chisq.test(observed)
        
        # Store the result
        comparison <- paste(categories[i], "vs", categories[j])
        results <- rbind(results, data.frame(Comparison = comparison, p.value = test_result$p.value))
      }
    }
  }
...
pairwise_chisq(con_mat_posed$table+1)

```

# also same categoruy comparison
pairwise_chisq <- function(matrix) {
  categories <- colnames(matrix)
  results <- data.frame(Comparison = character(), p.value = numeric(), stringsAsFactors = FALSE)
  
  for (i in 1:(nrow(matrix)-1)) {  # Adjusted loop to avoid redundant comparisons
    for (j in (i+1):ncol(matrix)) {  # Adjusted loop to avoid redundant comparisons
      # Create a 2x2 table for the chi-squared test
      observed <- matrix[c(i, j), c(i, j)]
      test_result <- chisq.test(observed)
...
# Perform the pairwise chi-squared tests
pairwise_results <- pairwise_chisq(con_mat_posed$table)
...
# Create the confusion matrix in one line
test_cm <- matrix(c(7, 7, 0, 7, 7, 0, 0, 0, 14), 
                           nrow = 3, 
                           byrow = TRUE,
                           dimnames = list("Reference" = c("angry", "happy", "sad"),
                                           "Prediction" = c("angry", "happy", "sad")))

pairwise_chisq(test_cm)
test_cm
...
test_cm<- matrix(c(7, 7, 1, 5, 9, 1, 1, 1, 14), ncol = 9)


# Define the categories for Reference and Prediction
categories <- list("Reference" = c("angry", "happy", "sad"),
                   "Prediction" = c("angry", "happy", "sad"))

# Create all possible pairs with Reference going vertically
all_pairs <- expand.grid(Prediction = categories$Prediction, Reference = categories$Reference)

# Create a vector with the format "Reference-Prediction"
names <- paste(all_pairs$Reference, all_pairs$Prediction, sep = "-")

# Print the vector
print(names)

colnames(test_cm) <- names
test_cm<- as.table(test_cm)

RVAideMemoire::chisq.multcomp(test_cm, p.method = "none")

race <- matrix(c(7, 7, 0, 5, 9, 0, 0, 0, 14),ncol=9)
colnames(race) <- c(1:9)
race<-as.table(race)
race
...
chi_res
library(RVAideMemoire)
chisq.multcomp(race+1, p.method = "none")

```
rcompanion::pairwiseNominalIndependence(con_mat_posed$table,
                                        fisher = TRUE)
...
tehre is a mistake ejre with the expression shuffle
```{r}
...
# 0.4766667
  ...
  # pvalue
mean(acc_test$acc_temp >= Acc_unshuff)
``` 
...
axis_titleimage <- c(
  # comp1 = "<img src='https://github.com/hcuve/halo_faces/raw/main/ggtextimgs/comp1n.png'
  #   width='100' /><br> 3",
  comp2 = "<img src='https://github.com/hcuve/halo_faces/raw/main/ggtextimgs/comp2n.png'
    width='100' /><br> 2",
  comp1 = "<img src='https://github.com/hcuve/halo_faces/raw/main/ggtextimgs/comp3n.png'
    width='100' /><br> 1"
)
...
boundaryplots$pc1_2_ggside+
  ...
  # xlab("Component (K)")+
   ...
   scale_y_continuous(
    ...
    # labels = labels) +
  theme(
     axis.title.x = element_markdown(color = "black", size = 16*(sf+.5)),
     axis.title.y = element_markdown(color = "black", size = 16*(sf+.5),angle = 0)
...
# 1500 but 900 seems to work - save this
...
nbprediction = predict(r.forest_mts, test_set)

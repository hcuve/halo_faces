...
df_OF_output_AUsW_unblind_posed_binned$AU05_r_Upper_lid_raiser
...
NMF::basismap(res_k3)
NMF::basismap(res_k3_max_norm)


NMF::coefmap(res_k3)
NMF::coefmap(res_k3_max_norm)
res_k3_max_norm
...
res_k3_max_norm@fit@H
unname((res_k3_max_norm@fit@H[1,]))
colnames(res_k3_max_norm@fit@H)

# Assuming `df` is your dataframe
values <- unname(df[1, ])
values_string <- paste(unname((res_k3_max_norm@fit@H[1,])), collapse = ", ")

 # Below is a simple R code that would take two vectors: one from Py-Feat AUs and one from your dataset. It will merge them, setting the values for non-existent AUs in your dataset to zero, and keeping the values from your dataset for the AUs that are present in both.

# Assuming res_k3_max_norm@fit@H is a matrix with AU activations,
# where each column corresponds to an AU from your dataset
...
# Extract the first row of H matrix from your NMF result
# This is assuming that `res_k3_max_norm@fit@H` is accessible and the first row can be extracted like this
myau_values <- as.numeric(res_k3_max_norm@fit@H[1, ])

# Create a named vector with zeros for all Py-Feat AUs
...
for (i in seq_along(my_aus)) {
  ...
  if (!is.na(au_index)) { # If the AU code exists in Py-Feat AUs
    merged_au_values[au_index] <- myau_values[i]
  }
}

# Resulting vector with zeros for non-existent AUs in your set
merged_au_values*100

merged_au_values_1<-round(merged_au_values*10)

merged_au_values_1

paste(unname((merged_au_values_1)), collapse = ", ")

...
ungroup()%>%
  ...
  gather(component, coef,-bin_frame,-output1)%>%
  group_by(component, bin_frame)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE) %>%
  ggplot(aes(as.numeric(bin_frame), coef))+

 geom_rect(aes(xmin = bin_frame, xmax = dplyr::lead(bin_frame), ymin = -Inf, ymax = Inf, fill = factor(output1)), 
            alpha = .1)  +
    geom_line(aes(group = component))+
  # geom_rect(aes(xmin=kmean_order-0.5, xmax=kmean_order+0.5))+ 
  # geom_smooth(se = F)+

...
group_by(filename, manual_clust_sp_disp,subject)%>%
  mutate(clust_frame_no_bin = n())%>%
group_by(filename, manual_clust_sp_disp, drug.placebo, expression,subject)%>%
  summarise_if(is.numeric, mean, na.rm  =TRUE)%>%
  subset(manual_clust_sp_disp == "transition state")%>%
  ggplot(aes(drug.placebo , clust_frame_no_bin))+
  # geom_jitter(width = .1, alpha = .1)+
  stat_summary(geom = "pointrange")+
...
group_by(filename, manual_clust_sp_disp, drug.placebo,component, expression)%>%
  summarise_if(is.numeric, mean, na.rm  =TRUE)%>%
  subset(manual_clust_sp_disp == "transition state")%>%
  ggplot(aes(expression, sp_avg_clust))+
  # geom_jitter(width = .1, alpha = .1)+
  stat_summary(geom = "pointrange")+
  facet_grid(~drug.placebo)+
  theme_classic()+
...
lmer_substates$clust_no <- glmer(clust_frame_no ~  drug.placebo * expression+
                              (1|subject),

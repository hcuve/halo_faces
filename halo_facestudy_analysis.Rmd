...
df_OF_output_AUsW_unblind_posed_binned1<- df_OF_output_AUsW_unblind_posed_binned%>%
  ungroup()%>%
  group_by(filename)%>%
  ungroup()%>%
  # subset(filename == "./cut_posed_angry_day1_p1.csv")%>%
...
spatiotemporal clustering using kmeans 
...
# for now loop oover people but fit the same k = 3

# create an id that matches what we have in the loop

df_OF_output_AUsW_unblind_posed_binned1
df_OF_output_AUsW_unblind_posed_binned2_nona<- na.omit(df_OF_output_AUsW_unblind_posed_binned1)
...
for (u in 1: length(unique_trial)) {
  # subset each trail on every run
   temp_df<- subset(df_OF_output_AUsW_unblind_posed_binned2_nona,filename == unique_trial[u])[,52:59]
...
df_OF_output_AUsW_unblind_posed_binned3_nona%>%
  subset(idno  == 1)%>%
    group_by(idno)%>%
    mutate(sp_avg = (k4_comp1_speed_ma+k4_comp2_speed_ma+k4_comp3_speed_ma+k4_comp4_speed_ma)/4,
           cuoff = (mean(sp_avg, na.rm = TRUE) + (2* mad(sp_avg, na.rm = TRUE)))) %>%
  ungroup()%>%
  select(c(2,52:59, 46:47,38:41,64))%>%
...
df_OF_output_AUsW_unblind_posed_binned3_nona %>%
  ...
  group_by(filename)%>%
   group_by(idno)%>%
    mutate(sp_avg = (k4_comp1_speed_ma+k4_comp2_speed_ma+k4_comp3_speed_ma+k4_comp4_speed_ma)/4)%>%
   mutate(comp_delta_avg = (k4_comp1_diff_ma +k4_comp2_diff_ma+k4_comp3_diff_ma+k4_comp4_diff_ma)/4)%>%
  # mutate(df, IVMean = rowMeans(select(df, starts_with("IV")), na.rm = TRUE))
  ...
  gather(component, coef,-bin_frame,-manual_clust_sp_disp,-idno,-manual_clust_sp_disp) %>%
  mutate(var_type = if_else(grepl("diff", component ), "diff",
                        if_else(grepl("speed", component ), "speed",  
                                if_else(grepl("sp_avg", component ), "avg_speed",  

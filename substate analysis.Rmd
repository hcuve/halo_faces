...
spoken_NMF_with_clusters6 <- spoken_NMF_int %>%
  group_by(filename) %>%
  subset(drug.placebo == "placebo")%>%
...
spoken_NMF_with_clusters6%>%
    # subset(idno == 30)%>%
  subset(filename == "./cut_spoken_happy_day1_p17.csv")%>%
...
mutate(speed = sp_avg,
  ...
  gather(variable, coef,-bin_frame,-filename,-cluster_across) %>%
  mutate(var_type = if_else(grepl("speed", variable), "2. speed",  
...
spoken_NMF_with_clusters6$cluster_label <- if_else(spoken_NMF_with_clusters6$cluster_across == 1, "relax",
                                                 if_else(spoken_NMF_with_clusters6$cluster_across == 2, "transition",
                                                         if_else(spoken_NMF_with_clusters6$cluster_across == 3,"sustain", NA)))
...
group_by(filename, cluster_across, expression, subject)%>%
    mutate(peak_speed =  sum(abs(sp_avg), na.rm = TRUE),
           max_displacement = sum(abs(comp_delta_avg), na.rm = TRUE))%>%
  group_by(filename)%>%
  # mutate(max_displacement = maxnorm(max_displacement),
  #       peak_speed = maxnorm(peak_speed) 
  #        )%>%
...
spoken_NMF_with_clusters6
spoken_NMF_with_clusters6<- spoken_NMF_with_clusters6%>%
...
max_displacement_norm =  maxnorm(max_displacement))%>%
 ...
 ggplot(aes(expression, entropy_clust_norm))+
  ...
  # unique(spoken_NMF_with_clusters6)
...
posed_spoken_NMF_with_clusters6_agg <-bind_rows(
  spoken_NMF_with_clusters6 [,c("filename","subject", "cluster_label","expression","posed_spoken","drug.placebo", "peak_speed", "max_displacement")],
  df_with_clusters6[,c("filename","subject", "cluster_label","expression","posed_spoken","drug.placebo", "peak_speed", "max_displacement")])%>%

...
group_by(filename, cluster_label, expression, subject)%>%
  ...
  group_by(filename)%>%
    ...
    summarise_if(is.numeric, mean, na.rm = T)%>%
    ggplot(aes(max_displacement, peak_speed))+
      geom_point()+
...
group_by(subject,filename, cluster_label)%>%
mutate(freq_clus = n())%>% # compute frequency of each state
  group_by(filename)%>%
  mutate(freq_clus_perc = freq_clus/n())%>% # compute percentage of eacch state e.g. duration
  group_by(filename, expression, subject)%>%
...
# Dataframe filtering
spoken_posed_nmf_filtered <- spoken_posed_nmf_agg[!spoken_posed_nmf_agg$subject %in% tmp_subj_to_remove, ]

spoken_posed_nmf_filtered<- spoken_posed_nmf_filtered%>%
  subset(expression!= "neutral")

spoken_posed_nmf_filtered$trans_entropy_perclust<-NULL
write_csv(spoken_posed_nmf_filtered, "spoken_posed_nmf_filtered.csv")
colnames(spoken_posed_nmf_filtered)
...
spoken_posed_nmf_onlycomplete<-  spoken_posed_nmf[!spoken_posed_nmf$subject %in% spoken_posed_nmf, ]%>%
  subset(expression!= "neutral")

unique(spoken_posed_nmf_onlycomplete$filename)

# spoken p30 angry
plts<- list()
plts_substate_posed<-
spoken_posed_nmf_onlycomplete%>%  
  subset(filename == "./cut_posed_angry_day1_p30.csv")%>%
mutate(speed = sp_avg,
       displacement = comp_delta_avg)%>%
  select(c(bin_frame,speed,displacement,comp1,comp2,comp3,cluster_label))%>%
  gather(variable, coef,-bin_frame,-cluster_label,-filename) %>%
  mutate(var_type = if_else(grepl("speed", variable), "2. speed",  
                                        if_else(grepl("displacement", variable), "3. displacement",  
 if_else(grepl("comp", variable ), "1. NMF-comp",NA)))) %>%
...
spoken_posed_nmf_onlycomplete%>%  
  subset(filename == "./cut_spoken_sad_day1_p30.csv")%>%
mutate(speed = sp_avg,
       displacement = comp_delta_avg)%>%
  select(c(bin_frame,speed,displacement,comp1,comp2,comp3,cluster_label))%>%
  gather(variable, coef,-bin_frame,-cluster_label,-filename) %>%

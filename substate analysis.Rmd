...
spoken_posed_nmf<- spoken_posed_nmf_entr
...
spoken_posed_nmf$trans_entropy_perclust
# 
...
# fixing issue tps://github.com/hcuve/halo_faces/issues/1
...
# Assuming spoken_posed_nmf is your data frame
unique(spoken_posed_nmf$cluster_label)
library(dplyr)
library(data.table)  # for rleid
library(entropy)
...
spoken_posed_nmf_entropy<- spoken_posed_nmf_filtered%>%
  ...
  mutate(entropy_clust = entropy::entropy(table(cluster_label))) %>%
    mutate(entropy_clust_norm =entropy_clust/(log2(3)))
  spoken_posed_nmf_entropy$entropy_clust_norm
...
rslt_stat_entropy<-lmer(log1p(entropy_clust_norm+.1)~ expression*posed_spoken +(1+posed_spoken+expression|subject),
     REML =FALSE,
...
```{r}
rslt_trans_entropy<-lmer(trans_entropy_norm~ expression*posed_spoken +(1+posed_spoken|subject),
     REML =FALSE,
...
rslt_stat_entropy<-lmer(entropy_clust_norm~ expression*posed_spoken+
...
anova(rslt_stat_entropy)
emmeans::emmeans(rslt_stat_entropy, pairwise~expression)
emmeans::emmeans(rslt_stat_entropy, pairwise~expression|posed_spoken)
...
emmeans::emmeans(rslt_stat_entropy, pairwise~posed_spoken|expression, adjust = "bonf")
# install.packages("report")
report::report(rslt_stat_entropy)
report::report(anova(rslt_stat_entropy))

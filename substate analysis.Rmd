...
mutate(speed = sp_avg,
       displacement = comp_delta_avg)%>%
  select(c(bin_frame,speed,displacement,acceleration, comp1,comp2,comp3,cluster_label))%>%
  gather(variable, coef,-bin_frame,-cluster_label,-filename) %>%
  mutate(var_type = if_else(grepl("speed", variable), "2. speed",  
                                        if_else(grepl("displacement", variable), "3. displacement",  
 if_else(grepl("comp", variable ), "1. NMF-comp","acceleration")))) %>%
...
(plts_substate_posed+ plts_substate_spoken)/ (plts_substate_posed_HM/plts_substate_spok_HM)+

...
acceleration_avg_norm =  maxnorm(acceleration_avg))%>%
  ...
  group_by(filename, cluster_label, cluster_count_group)%>%
...
interactions::sim_margins(rslt_speedavg, pred = expression)
...
emmeans::emmeans(rslt_speedavg, pairwise~posed_spoken, adjust = "bonf")
...
emmeans::emmeans(rslt_speedavg, pairwise~posed_spoken|expression, adjust = "bonf")
...
```{r}
rslt_clust_dur_count_perc<-lmer(clust_dur_count_perc~ expression*posed_spoken*cluster_label +(1|subject),
     REML =FALSE,
     data = spoken_posed_nmf_filtered_agg)
...
rslt_trans_entropy<-lmer(trans_entropy~ expression*posed_spoken +(1|subject),
     REML =FALSE,
...
spoken_posed_nmf_filtered_agg%>%
  group_by(subject,expression,cluster_label, posed_spoken)%>%
  summarise_if(is.numeric, mean, na.rm = T)%>%
  ggplot(aes(cluster_label , speed, 
             color = posed_spoken))+
  # geom_split_violin(color = FALSE, alpha = .3)+
  # geom_smooth(aes(group = expression), method = 'lm', se = F, 
  #             color = "gray80", alpha = .5,size = .5)+
  geom_jitter(width = .1, alpha = .2)+
  # geom_point(fill = NA, pch = 21, position = position_jitterdodge(),width = .01, alpha = .5)+
  # stat_summary(aes(group = posed_spoken,colour = posed_spoken),fun = mean, geom = 'pointrange', width = 0.1, size = .2,
  #              # color = "black", 
  #              alpha = .8,position=position_dodge(width=0.3)) +
    facet_grid(~expression)+
  # ggside::geom_ysidedensity()+
  ggside::geom_ysidedensity(aes(x=stat(density), fill = expression), alpha = .5) +
  theme_classic()+
  p$graphstyle_int
